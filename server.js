...
const FINAL_ALLOWED_EMAILS = (process.env.FINAL_ALLOWED_EMAILS || '')
  .split(',').map((e) => e.trim().toLowerCase()).filter(Boolean);
const googleClient = GOOGLE_CLIENT_ID ? new OAuth2Client(GOOGLE_CLIENT_ID) : null;
function finalConfigured() { return !!(googleClient && FINAL_ALLOWED_EMAILS.length); }

if (!process.env.ACCESS_CODE) console.warn('[warn] ACCESS_CODE not set — using default "magic2026". Set it in Render.');
if (!finalConfigured()) console.warn('[warn] 파이널 모의고사(C): GOOGLE_CLIENT_ID / FINAL_ALLOWED_EMAILS not set — students will see the setup guide message.');
...
app.post('/api/final/verify', async (req, res) => {
  ...
  try {
    ...
    if (!FINAL_ALLOWED_EMAILS.includes(email)) return res.status(403).json({ ok: false, error: 'not-allowed', email });
