...
app.get('/api/config', (req, res) => {
  ...
  const hasAccess = isAdmin || verifyToken(token, 'access');
  if (!hasAccess) return res.status(401).json({ ok: false, error: 'locked' });
  if (isAdmin) return res.json(liveConfig);
  const studentView = JSON.parse(JSON.stringify(liveConfig));
  studentView.globals.exam = '';        // hide final-exam link from ordinary students
  studentView.finalAllowedEmails = [];  // never expose the student email list
