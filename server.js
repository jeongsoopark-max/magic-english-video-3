// server.js — signaling server for the small-scale video classroom
//
// Responsibilities:
//   1. Serve the static client (public/).
//   2. Relay WebRTC signaling messages (offer/answer/ICE) between peers
//      in the same room. The server never touches audio/video media —
//      that flows directly peer-to-peer once the connection is set up.
//   3. Track who is in each room and enforce a max room size.
//   4. Relay lightweight text chat messages within a room.
//   5. Optional "waiting room": whoever joins a room first (when nobody is
//      in it) becomes that room's host for as long as they stay connected.
//      If they turned on approval, later joiners sit in a pending queue
//      until the host admits or denies them.

const path = require('path');

// 서버 전체가 죽는 것을 막는 마지막 안전장치. 개별 API/소켓 핸들러는 각자
// try/catch로 방어하지만, 혹시 놓친 곳이 있어도 여기서 잡아서 로그만 남기고
// 프로세스는 계속 살아있게 한다 — 수업 중 서버가 죽으면 그 순간 접속해 있던
// 모든 사람의 화상 연결이 동시에 끊기기 때문에, 개별 요청 실패보다 전체
// 다운타임이 훨씬 나쁘다.
process.on('unhandledRejection', (err) => {
  console.error('[fatal-guard] unhandled rejection (server kept running):', err && err.stack || err);
});
process.on('uncaughtException', (err) => {
  console.error('[fatal-guard] uncaught exception (server kept running):', err && err.stack || err);
});
const express = require('express');
const http = require('http');
const { Server } = require('socket.io');

const PORT = process.env.PORT || 3000;
const MAX_ROOM_SIZE = 10; // small group class ceiling for mesh WebRTC

const app = express();
const server = http.createServer(app);
const io = new Server(server, { maxHttpBufferSize: 5e6 });

const MAX_PDF_BYTES = 25 * 1024 * 1024; // 25 MB cap per upload
// How much PDF a single room may hold at once. PDF buffers live in memory until
// the room fully empties, so without a ceiling a long class that uploads several
// PDFs keeps piling them up. These limits are generous but stop runaway growth.
const MAX_PDFS_PER_ROOM = 12;
const MAX_PDF_TOTAL_BYTES = 80 * 1024 * 1024; // 80 MB of PDF buffers per room

// Teacher uploads a PDF for a room (raw binary body). Stored in memory and
// served back to every participant, who each render it locally with PDF.js.
app.post('/upload-pdf/:roomId', express.raw({ type: 'application/pdf', limit: MAX_PDF_BYTES }), (req, res) => {
  const roomId = req.params.roomId;
  if (!roomId || !rooms.has(roomId)) {
    return res.status(404).json({ ok: false, error: 'no-such-room' });
  }
  if (!req.body || !req.body.length) {
    return res.status(400).json({ ok: false, error: 'empty' });
  }
  let pdfs = roomPdfs.get(roomId);
  if (!pdfs) { pdfs = new Map(); roomPdfs.set(roomId, pdfs); }
  // Reject the upload if this room is already holding too much PDF data, so a
  // long class (or an accidental re-upload loop) can't balloon memory.
  let heldBytes = 0;
  pdfs.forEach((e) => { heldBytes += e.buffer.length; });
  if (pdfs.size >= MAX_PDFS_PER_ROOM || heldBytes + req.body.length > MAX_PDF_TOTAL_BYTES) {
    return res.status(413).json({ ok: false, error: 'pdf-budget-exceeded' });
  }
  const pdfId = 'pdf-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 7);
  pdfs.set(pdfId, { buffer: Buffer.from(req.body), contentType: 'application/pdf' });
  res.json({ ok: true, pdfId });
});

// Any participant fetches a room's PDF to render it.
app.get('/room-pdf/:roomId/:pdfId', (req, res) => {
  const pdfs = roomPdfs.get(req.params.roomId);
  const entry = pdfs && pdfs.get(req.params.pdfId);
  if (!entry) return res.status(404).send('not found');
  res.set('Content-Type', entry.contentType);
  res.set('Cache-Control', 'no-store');
  res.send(entry.buffer);
});

// ---------------------------------------------------------------------------
// Server-side teacher admin: password check + shared class/link config.
//
// The password lives in an environment variable (ADMIN_PASSWORD) so it is never
// shipped to the browser and can't be read from page source. Login is verified
// on the server, which hands back a short-lived signed token. The editable
// config (class links + global links) is stored in config.json and served to
// every visitor, so a change made by the teacher reaches ALL student devices.
//
// NOTE on Render's Free tier: the filesystem is ephemeral, so config.json edits
// made at runtime survive until the service restarts / spins down / redeploys.
// For permanent changes, edit the committed config.json (or use a paid plan
// with a persistent disk). The links rarely change, so this is usually fine.
// ---------------------------------------------------------------------------
const fs = require('fs');
const crypto = require('crypto');
const { OAuth2Client } = require('google-auth-library');
const db = require('./db');
const attendance = require('./attendance');
const analytics = require('./analytics');

const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'magic777';
const ADMIN_SECRET = process.env.ADMIN_SECRET || crypto.randomBytes(32).toString('hex');
const TOKEN_TTL_MS = 8 * 60 * 60 * 1000; // teacher stays logged in for 8 hours
const CONFIG_PATH = path.join(__dirname, 'config.json');

// Emails allowed to open 파이널 모의고사 (C방식). Seeded from the env var, then
// editable in the teacher's admin panel (stored in config.json).
const ENV_FINAL_EMAILS = (process.env.FINAL_ALLOWED_EMAILS || '')
  .split(',').map((e) => e.trim().toLowerCase()).filter(Boolean);

const DEFAULT_CONFIG = {
  classes: {
    basic:        { meetLink: 'class.html?room=basic',        playlistLink: 'https://drive.google.com/drive/folders/1RTTcFvRGGNEUFSi0VLXkin-GarBw9hid', driveLink: 'https://drive.google.com/drive/folders/12iMQnFsQagQnPVW7Q2RcISGCaOwSMQod?usp=drive_link' },
    grammar:      { meetLink: 'class.html?room=grammar',      playlistLink: 'https://drive.google.com/drive/folders/1PX-DM6vB9Mugq7hYTW-XDHsoyViikIho', driveLink: 'https://drive.google.com/drive/folders/1sJplWPPBv0F2FSmq7TWEZVDJPBh6v-L9?usp=drive_link' },
    intermediate: { meetLink: 'class.html?room=intermediate', playlistLink: 'https://drive.google.com/drive/folders/13Di9NJJGKJTD2lHsS95RN5psvb7U6RaY', driveLink: 'https://drive.google.com/drive/folders/1vNuffBohWAwdWav4nFpoW2xLHI2pmAY_?usp=drive_link' },
    advanced:     { meetLink: 'class.html?room=advanced',     playlistLink: 'https://drive.google.com/drive/folders/1hq-gLoaTmssCxR_bryueuXijGMTCsR6W', driveLink: 'https://drive.google.com/drive/folders/1DGvWoUFCf367Dw21llcwDCy2dNt0P_uS?usp=drive_link' },
    private:      { meetLink: 'class.html?room=private',       playlistLink: 'https://drive.google.com/drive/folders/1hK_9ud_KU77UJ3M9HpD7rM7lHxYCDiLg', driveLink: 'https://drive.google.com/drive/folders/19LOEvdvTW0gPH6jd3Sin_wB59lmi4_qn?usp=drive_link' }
  },
  globals: {
    vocab: 'https://magic-vocabulary-sjlherfhfppdwp3tgvppqe.streamlit.app/',
    exam: 'https://drive.google.com/drive/folders/12m1z1Y64CLA27nGrwpDsW8eMNz62VvD8',
    studentForm: 'https://forms.gle/1FvJroxUVWzswa5v9',
    adminFormFolder: 'https://drive.google.com/drive/folders/1HuUKm9223tywPEbtooe33A4-8BQdF0L_'
  },
  finalAllowedEmails: ENV_FINAL_EMAILS.slice()
};
const CLASS_KEYS = Object.keys(DEFAULT_CONFIG.classes);
const GLOBAL_KEYS = Object.keys(DEFAULT_CONFIG.globals);

// Keep only known keys, force strings, cap length; fall back to defaults.
function sanitizeConfig(input) {
  const clean = (v, def) => (typeof v === 'string' ? v.trim().slice(0, 2000) : def);
  const out = { classes: {}, globals: {}, finalAllowedEmails: [] };
  CLASS_KEYS.forEach((k) => {
    const src = (input && input.classes && input.classes[k]) || {};
    const def = DEFAULT_CONFIG.classes[k];
    out.classes[k] = {
      meetLink: clean(src.meetLink, def.meetLink),
      playlistLink: clean(src.playlistLink, def.playlistLink),
      driveLink: clean(src.driveLink, def.driveLink)
    };
  });
  GLOBAL_KEYS.forEach((k) => {
    const src = (input && input.globals) || {};
    out.globals[k] = clean(src[k], DEFAULT_CONFIG.globals[k]);
  });
  // Allow-list of emails: lowercase, trimmed, unique, basic shape, capped.
  const rawEmails = (input && Array.isArray(input.finalAllowedEmails))
    ? input.finalAllowedEmails
    : DEFAULT_CONFIG.finalAllowedEmails;
  const seen = new Set();
  out.finalAllowedEmails = (rawEmails || [])
    .map((e) => (typeof e === 'string' ? e.trim().toLowerCase() : ''))
    .filter((e) => e && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(e) && e.length <= 254)
    .filter((e) => (seen.has(e) ? false : (seen.add(e), true)))
    .slice(0, 500);
  return out;
}

function loadConfig() {
  try { return sanitizeConfig(JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'))); }
  catch (e) { return JSON.parse(JSON.stringify(DEFAULT_CONFIG)); }
}
function saveConfig(cfg) {
  try { fs.writeFileSync(CONFIG_PATH, JSON.stringify(cfg, null, 2)); return true; }
  catch (e) { console.error('config save failed:', e.message); return false; }
}
let liveConfig = loadConfig();

// Minimal signed token (HMAC) so we don't need extra dependencies.
// `kind` distinguishes an "admin" (teacher) token from an "access" (student) token.
function signToken(kind) {
  const payload = kind + '.' + (Date.now() + TOKEN_TTL_MS);
  const sig = crypto.createHmac('sha256', ADMIN_SECRET).update(payload).digest('hex');
  return payload + '.' + sig;
}
function verifyToken(token, kind) {
  if (typeof token !== 'string') return false;
  const parts = token.split('.');
  if (parts.length !== 3 || parts[0] !== kind) return false;
  const payload = parts[0] + '.' + parts[1];
  const expected = crypto.createHmac('sha256', ADMIN_SECRET).update(payload).digest('hex');
  const a = Buffer.from(parts[2]); const b = Buffer.from(expected);
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) return false;
  return Date.now() < Number(parts[1]);
}
// A short-lived, room-scoped token handed to anyone already admitted. If their
// socket blips and reconnects, the client presents this on re-join so we can let
// them straight back in WITHOUT making the teacher approve them again.
const RESUME_TTL_MS = 3 * 60 * 1000; // 3 minutes is plenty for a network blip
function signResume(roomId) {
  const payload = 'resume.' + roomId + '.' + (Date.now() + RESUME_TTL_MS);
  const sig = crypto.createHmac('sha256', ADMIN_SECRET).update(payload).digest('hex');
  return payload + '.' + sig;
}
function verifyResume(token, roomId) {
  if (typeof token !== 'string') return false;
  const parts = token.split('.');
  if (parts.length !== 4 || parts[0] !== 'resume' || parts[1] !== roomId) return false;
  const payload = parts[0] + '.' + parts[1] + '.' + parts[2];
  const expected = crypto.createHmac('sha256', ADMIN_SECRET).update(payload).digest('hex');
  const a = Buffer.from(parts[3]); const b = Buffer.from(expected);
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) return false;
  return Date.now() < Number(parts[2]);
}

function safeEqual(a, b) {
  const ba = Buffer.from(typeof a === 'string' ? a : '');
  const bb = Buffer.from(typeof b === 'string' ? b : '');
  return ba.length === bb.length && crypto.timingSafeEqual(ba, bb);
}
function passwordMatches(pw) { return safeEqual(pw, ADMIN_PASSWORD); }
function bearer(req) {
  const h = req.headers['authorization'] || '';
  return h.startsWith('Bearer ') ? h.slice(7) : '';
}

// --- Site access (A방식): a shared code gates everything except the landing page.
const ACCESS_CODE = process.env.ACCESS_CODE || 'magic2026';

// --- 파이널 모의고사 (C방식): Google sign-in restricted to allow-listed emails.
// The allow-list itself lives in the config (liveConfig.finalAllowedEmails) and
// is editable from the teacher's admin panel; it's seeded from FINAL_ALLOWED_EMAILS.
const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || '';
const googleClient = GOOGLE_CLIENT_ID ? new OAuth2Client(GOOGLE_CLIENT_ID) : null;
function allowedEmails() { return (liveConfig && liveConfig.finalAllowedEmails) || []; }
function finalConfigured() { return !!(googleClient && allowedEmails().length); }

if (!process.env.ACCESS_CODE) console.warn('[warn] ACCESS_CODE not set — using default "magic2026". Set it in Render.');
if (!googleClient) console.warn('[warn] 파이널 모의고사(C): GOOGLE_CLIENT_ID not set — students will see the setup guide message.');

app.use(express.json({ limit: '64kb' }));

// 출석 / 결석 학부모 알림 API (관리자 토큰 필요)
app.use('/api/admin/attendance', attendance.buildRouter({ verifyToken, bearer }));
// 폰 자동화 앱(MacroDroid 등)이 문자 수신 시 호출하는 웹훅 (SMS_INTAKE_SECRET로 인증)
app.use('/api/sms-intake', attendance.buildWebhookRouter());

// 방문 통계: 수집은 방문자 브라우저가 직접 부르므로 열어 두고,
// 조회는 출결 API와 같은 관리자 토큰으로 막는다.
app.use('/api/track', analytics.buildTrackRouter());
app.use('/api/admin/stats', analytics.buildStatsRouter({ verifyToken, bearer }));

// Student site access: verify the shared code, return a short-lived token.
app.post('/api/access/login', (req, res) => {
  const ok = safeEqual(req.body && req.body.code, ACCESS_CODE);
  setTimeout(() => {
    if (ok) res.json({ ok: true, token: signToken('access') });
    else res.status(401).json({ ok: false, error: 'bad-code' });
  }, ok ? 0 : 400);
});

// Shared link config. Requires a valid access (student) OR admin (teacher) token.
// Ordinary students never receive the 파이널 모의고사 link here — it's released only
// after the Google allow-list check below. The teacher (admin token) gets everything.
app.get('/api/config', (req, res) => {
  res.set('Cache-Control', 'no-store');
  const token = bearer(req);

  // Teacher (admin token) gets the full config, including the final-exam link
  // and the allow-list.
  if (verifyToken(token, 'admin')) return res.json(liveConfig);

  // Everyone else gets the student view. The site access gate (A방식) is disabled
  // in the client — index.html unlocks unconditionally — so students never hold an
  // access token and requiring one here only produced a 401 on every page load,
  // which meant config.json edits never reached students. The links below are
  // already present in the page source, so serving them here exposes nothing new.
  // The two genuinely sensitive fields are still stripped, and the 파이널 모의고사
  // link is released only after the Google allow-list check in /api/final/verify.
  const studentView = JSON.parse(JSON.stringify(liveConfig));
  if (studentView.globals) studentView.globals.exam = '';  // hide final-exam link
  studentView.finalAllowedEmails = [];                     // never expose the email list
  res.json(studentView);
});

// Teacher login → verify password, return an admin token + the full config.
app.post('/api/admin/login', (req, res) => {
  const ok = passwordMatches(req.body && req.body.password);
  setTimeout(() => {                      // small delay slows brute-forcing
    if (ok) res.json({ ok: true, token: signToken('admin'), config: liveConfig });
    else res.status(401).json({ ok: false, error: 'bad-password' });
  }, ok ? 0 : 400);
});

// Teacher save → requires a valid admin token; writes the shared config for everyone.
app.post('/api/admin/config', (req, res) => {
  if (!verifyToken(req.body && req.body.token, 'admin')) {
    return res.status(401).json({ ok: false, error: 'unauthorized' });
  }
  liveConfig = sanitizeConfig(req.body && req.body.config);
  const persisted = saveConfig(liveConfig);
  res.json({ ok: true, config: liveConfig, persisted });
});

// 파이널 모의고사 gate info (public): is C방식 configured, and the OAuth client id
// (the client id is not a secret — it is meant to be used in the browser).
app.get('/api/final/info', (req, res) => {
  res.set('Cache-Control', 'no-store');
  res.json({ configured: finalConfigured(), clientId: GOOGLE_CLIENT_ID });
});

// 파이널 모의고사 access: verify the Google ID token + allow-list, return the link.
app.post('/api/final/verify', async (req, res) => {
  if (!finalConfigured()) return res.status(503).json({ ok: false, error: 'not-configured' });
  const credential = req.body && req.body.credential;
  if (typeof credential !== 'string' || !credential) {
    return res.status(400).json({ ok: false, error: 'no-credential' });
  }
  try {
    const ticket = await googleClient.verifyIdToken({ idToken: credential, audience: GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload() || {};
    const email = (payload.email || '').toLowerCase();
    if (!email || !payload.email_verified) return res.status(401).json({ ok: false, error: 'unverified' });
    if (!allowedEmails().includes(email)) return res.status(403).json({ ok: false, error: 'not-allowed', email });
    return res.json({ ok: true, link: liveConfig.globals.exam || '', email });
  } catch (e) {
    return res.status(401).json({ ok: false, error: 'invalid-token' });
  }
});

// ===== 듣고 따라 읽기(2B): Azure 발음평가 토큰 + 발음 기록 (기존 파이널 인증 재사용) =====
const AZURE_SPEECH_KEY = process.env.AZURE_SPEECH_KEY || '';
const AZURE_SPEECH_REGION = process.env.AZURE_SPEECH_REGION || '';

// 듣고 따라 읽기(2B): 파이널 명단에 있는 학생에게만 Azure 단기 토큰(약 10분)을 발급.
app.post('/api/azure/token', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  if (!finalConfigured()) return res.status(503).json({ ok: false, error: 'not-configured' });
  if (!AZURE_SPEECH_KEY || !AZURE_SPEECH_REGION) {
    return res.status(503).json({ ok: false, error: 'azure-not-configured' });
  }
  const credential = req.body && req.body.credential;
  if (typeof credential !== 'string' || !credential) {
    return res.status(400).json({ ok: false, error: 'no-credential' });
  }
  try {
    const ticket = await googleClient.verifyIdToken({ idToken: credential, audience: GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload() || {};
    const email = (payload.email || '').toLowerCase();
    if (!email || !payload.email_verified) return res.status(401).json({ ok: false, error: 'unverified' });
    if (!allowedEmails().includes(email)) return res.status(403).json({ ok: false, error: 'not-allowed', email });

    const r = await fetch(
      `https://${AZURE_SPEECH_REGION}.api.cognitive.microsoft.com/sts/v1.0/issueToken`,
      { method: 'POST', headers: { 'Ocp-Apim-Subscription-Key': AZURE_SPEECH_KEY, 'Content-Length': '0' } }
    );
    if (!r.ok) return res.status(502).json({ ok: false, error: 'azure-token-failed' });
    const token = await r.text();
    return res.json({ ok: true, token, region: AZURE_SPEECH_REGION, email });
  } catch (e) {
    return res.status(401).json({ ok: false, error: 'invalid-token' });
  }
});


// ============================================================================
// 발음 평가 기록 (교사용).  위 Azure 블록과 같은 위치(=/api/final/verify 아래)에
// 이어서 두면 됩니다. 기존 verifyToken / bearer / googleClient / allowedEmails 재사용.
// 무료 Render는 재시작 시 메모리가 초기화되므로, 장기 보관이 필요하면 교사 페이지에서
// CSV로 내보내 두세요.
// ============================================================================
const pronRecords = [];
const PRON_MAX = 5000;

// 학생: 평가 1건 저장 (credential로 신원·파이널 명단 확인)
app.post('/api/pron/record', async (req, res) => {
  if (!finalConfigured()) return res.status(503).json({ ok: false });
  const b = req.body || {};
  const credential = b.credential;
  if (typeof credential !== 'string' || !credential) return res.status(400).json({ ok: false });
  try {
    const ticket = await googleClient.verifyIdToken({ idToken: credential, audience: GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload() || {};
    const email = (payload.email || '').toLowerCase();
    if (!email || !payload.email_verified) return res.status(401).json({ ok: false });
    if (!allowedEmails().includes(email)) return res.status(403).json({ ok: false });
    const s = b.scores || {};
    const clamp = (v) => { v = Number(v); return isFinite(v) ? Math.max(0, Math.min(100, Math.round(v))) : null; };
    pronRecords.push({
      email,
      name: String(payload.name || '').slice(0, 60),
      ts: Date.now(),
      sentence: String(b.sentence || '').slice(0, 300),
      pron: clamp(s.pron), acc: clamp(s.acc), flu: clamp(s.flu), comp: clamp(s.comp), pros: clamp(s.pros)
    });
    if (pronRecords.length > PRON_MAX) pronRecords.splice(0, pronRecords.length - PRON_MAX);
    res.json({ ok: true });
  } catch (e) { res.status(401).json({ ok: false }); }
});

// 교사: 전체 기록 조회 (admin 토큰 필요)
app.get('/api/pron/records', (req, res) => {
  res.set('Cache-Control', 'no-store');
  if (!verifyToken(bearer(req), 'admin')) return res.status(401).json({ ok: false, error: 'unauthorized' });
  res.json({ ok: true, records: pronRecords });
});

// 교사: 기록 비우기 (admin 토큰 필요)
app.post('/api/pron/clear', (req, res) => {
  if (!verifyToken(req.body && req.body.token, 'admin')) return res.status(401).json({ ok: false });
  pronRecords.length = 0;
  res.json({ ok: true });
});


// ICE servers for WebRTC. Always returns a STUN server, plus a TURN relay if
// one is configured. TURN is what fixes "갑자기 끊김/연결 안 됨" on mobile carriers
// (symmetric NAT / CGNAT), school/office firewalls and strict in-app browsers.
//
// Providers are tried in order; the first one that works wins:
//
//   (A) Cloudflare Realtime TURN (current): set TURN_KEY_ID + TURN_KEY_API_TOKEN.
//       1,000 GB/month free, then $0.05/GB. The key is a long-lived master
//       secret that never leaves the server; we mint short-lived credentials
//       from it and cache them until shortly before they expire.
//       Optional: TURN_TTL_SECONDS (default 14400 = 4h).
//   (B) Metered ephemeral creds (legacy): set METERED_DOMAIN + METERED_API_KEY.
//       Kept as a fallback; the free tier is only 500 MB/month as of 2026.
//   (C) Static creds (coturn / any provider): set TURN_URLS (comma-separated),
//       TURN_USERNAME, TURN_CREDENTIAL.
//
// If every provider fails we still return STUN, so most students connect
// normally — only those behind restrictive NAT lose the relay path.

const CF_TURN_KEY_ID = process.env.TURN_KEY_ID;
const CF_TURN_API_TOKEN = process.env.TURN_KEY_API_TOKEN;
const CF_TURN_TTL = Number(process.env.TURN_TTL_SECONDS || 14400);
// Refresh 10 minutes before expiry so a running class never hits a dead credential.
const CF_REFRESH_MARGIN_MS = 10 * 60 * 1000;

let cfTurnCache = null;      // { iceServers, expiresAt }
let cfTurnInflight = null;   // de-dupes bursts when a whole class joins at once

async function getCloudflareIceServers() {
  if (cfTurnCache && Date.now() < cfTurnCache.expiresAt) return cfTurnCache.iceServers;
  if (cfTurnInflight) return cfTurnInflight;

  const url = `https://rtc.live.cloudflare.com/v1/turn/keys/${CF_TURN_KEY_ID}/credentials/generate-ice-servers`;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 8000);

  cfTurnInflight = (async () => {
    try {
      const r = await fetch(url, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${CF_TURN_API_TOKEN}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ttl: CF_TURN_TTL, customIdentifier: 'magic-english' }),
        signal: controller.signal,
      });
      if (!r.ok) {
        const body = await r.text().catch(() => '');
        throw new Error(`HTTP ${r.status} ${body.slice(0, 160)}`);
      }
      const data = await r.json();
      if (!data || !Array.isArray(data.iceServers) || !data.iceServers.length) {
        throw new Error('response had no iceServers');
      }
      cfTurnCache = {
        iceServers: data.iceServers,
        expiresAt: Date.now() + CF_TURN_TTL * 1000 - CF_REFRESH_MARGIN_MS,
      };
      console.log('[turn] Cloudflare credentials issued (ttl %ds)', CF_TURN_TTL);
      return cfTurnCache.iceServers;
    } finally {
      clearTimeout(timer);
      cfTurnInflight = null;
    }
  })();

  return cfTurnInflight;
}

app.get('/api/turn', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  const iceServers = [{ urls: 'stun:stun.l.google.com:19302' }];

  // (A) Cloudflare Realtime TURN.
  if (CF_TURN_KEY_ID && CF_TURN_API_TOKEN && typeof fetch === 'function') {
    try {
      const cf = await getCloudflareIceServers();
      return res.json({ iceServers: iceServers.concat(cf), source: 'cloudflare' });
    } catch (e) {
      console.warn('[turn] Cloudflare failed, falling back:', e.message);
    }
  }

  // (B) Metered ephemeral credentials (legacy).
  const meteredDomain = process.env.METERED_DOMAIN;
  const meteredKey = process.env.METERED_API_KEY;
  if (meteredDomain && meteredKey && typeof fetch === 'function') {
    try {
      const r = await fetch(`https://${meteredDomain}/api/v1/turn/credentials?apiKey=${encodeURIComponent(meteredKey)}`);
      if (r.ok) {
        const list = await r.json();
        if (Array.isArray(list) && list.length) {
          return res.json({ iceServers: iceServers.concat(list), source: 'metered' });
        }
      }
    } catch (e) { /* fall through to static / stun-only */ }
  }

  // (C) Static TURN credentials.
  const turnUrls = (process.env.TURN_URLS || '').split(',').map((s) => s.trim()).filter(Boolean);
  if (turnUrls.length && process.env.TURN_USERNAME && process.env.TURN_CREDENTIAL) {
    iceServers.push({ urls: turnUrls, username: process.env.TURN_USERNAME, credential: process.env.TURN_CREDENTIAL });
    return res.json({ iceServers, source: 'static' });
  }

  console.warn('[turn] no TURN provider configured — STUN only');
  res.json({ iceServers, source: 'stun-only' });
});

app.use(express.static(path.join(__dirname, 'public')));

// roomId -> Map<socketId, { name }>   (admitted members, i.e. actually in the call)
const rooms = new Map();
// roomId -> socketId of the current host (the first person who opened the room)
const roomHosts = new Map();
// roomId -> { waitingRoomEnabled: boolean }
const roomSettings = new Map();
// roomId -> Map<socketId, { name }>   (asked to join, waiting on the host)
const roomPending = new Map();
// roomId -> socketId of whoever is currently screen-sharing (if anyone)
const roomSharingPeer = new Map();

// ---- Shared multi-page whiteboard --------------------------------------
// Each room's board is an ordered list of PAGES. A page is either blank or
// backed by one page of an uploaded PDF. Every page keeps its own pen strokes
// so handwriting stays put when you flip pages. Late joiners get the whole
// thing (page list + strokes + which page is showing) so their board matches.
//
// roomId -> {
//   active: bool,
//   currentPage: number,
//   pages: [ { id, type:'blank'|'pdf', pdfId?, pageIndex?, aspect, strokes: Map } ],
// }
const roomWhiteboard = new Map();
// roomId -> Map<pdfId, { buffer: Buffer, contentType: string }>
const roomPdfs = new Map();

// --- Whiteboard memory budget --------------------------------------------
// A room never empties during a live class, so its board is only cleaned up
// when everyone finally leaves. Handwriting is stored as { x, y } point
// objects, and without ceilings a 90-minute class accumulates millions of
// them and exhausts the JS heap. These caps bound how much the board can grow.
const MAX_POINTS_PER_STROKE = 5000;   // a single continuous pen stroke
const MAX_POINTS_PER_ROOM   = 250000; // total stored handwriting points per room
const MAX_PAGES_PER_ROOM    = 300;    // blank + PDF pages combined

// Count all stored handwriting points across every page of a board.
function whiteboardPointCount(wb) {
  let n = 0;
  for (const pg of wb.pages) {
    for (const s of pg.strokes.values()) n += s.points.length;
  }
  return n;
}

// Keep a board under its point budget by evicting the oldest strokes first
// (earliest page, oldest stroke). Called on a throttle from wb-stroke so the
// cost of the recount is amortized. Old handwriting on already-covered pages
// disappearing is an acceptable trade for a server that stays alive.
function trimWhiteboard(wb) {
  let total = whiteboardPointCount(wb);
  if (total <= MAX_POINTS_PER_ROOM) return;
  for (const pg of wb.pages) {
    if (total <= MAX_POINTS_PER_ROOM) break;
    for (const key of Array.from(pg.strokes.keys())) {
      if (total <= MAX_POINTS_PER_ROOM) break;
      const s = pg.strokes.get(key);
      total -= s.points.length;
      pg.strokes.delete(key);
    }
  }
}

function getWhiteboard(roomId) {
  let wb = roomWhiteboard.get(roomId);
  if (!wb) {
    wb = { active: false, currentPage: 0, view: { zoom: 1, panX: 0, panY: 0 }, pages: [newBlankPage()] };
    roomWhiteboard.set(roomId, wb);
  }
  return wb;
}

function newBlankPage() {
  return {
    id: 'pg-' + Math.random().toString(36).slice(2, 9),
    type: 'blank',
    aspect: 4 / 3,
    strokes: new Map(),
  };
}

// Serialize the board for sending to a client (strokes as arrays, no Maps).
function whiteboardSnapshot(roomId) {
  const wb = getWhiteboard(roomId);
  return {
    active: wb.active,
    currentPage: wb.currentPage,
    view: wb.view || { zoom: 1, panX: 0, panY: 0 },
    pages: wb.pages.map((pg) => ({
      id: pg.id,
      type: pg.type,
      pdfId: pg.pdfId,
      pageIndex: pg.pageIndex,
      aspect: pg.aspect,
      strokes: Array.from(pg.strokes.values()),
    })),
  };
}

// Lightweight page list (no strokes) for broadcasting structure changes.
function pageListPayload(roomId) {
  const wb = getWhiteboard(roomId);
  return {
    currentPage: wb.currentPage,
    pages: wb.pages.map((pg) => ({
      id: pg.id, type: pg.type, pdfId: pg.pdfId,
      pageIndex: pg.pageIndex, aspect: pg.aspect,
    })),
  };
}


function getRoomMembers(roomId) {
  return rooms.get(roomId) || new Map();
}

function roomSummary(roomId) {
  const members = getRoomMembers(roomId);
  return Array.from(members.entries())
    .filter(([, info]) => !info.whiteboardOnly)
    .map(([id, info]) => ({ id, name: info.name }));
}

// Number of real video participants (excludes the teacher's whiteboard-only
// tablet, which shouldn't count against the mesh size limit or the count).
function videoMemberCount(roomId) {
  const members = getRoomMembers(roomId);
  let n = 0;
  members.forEach((info) => { if (!info.whiteboardOnly) n += 1; });
  return n;
}

function broadcastPendingList(roomId) {
  const hostId = roomHosts.get(roomId);
  if (!hostId) return;
  const pending = roomPending.get(roomId) || new Map();
  const list = Array.from(pending.entries()).map(([id, info]) => ({ id, name: info.name }));
  io.to(hostId).emit('pending-list', list);
}

io.on('connection', (socket) => {
  let joinedRoom = null;   // set once actually admitted into a room
  let pendingRoom = null;  // set while sitting in a room's waiting queue
  let attendanceId = null; // set once this socket's join is logged for attendance

  socket.on('join-room', ({ roomId, name, requireApproval, whiteboardOnly, resumeToken }, ack) => {
    if (typeof ack !== 'function') return;
    if (!roomId || typeof roomId !== 'string') {
      return ack({ ok: false, error: 'invalid-room' });
    }
    const cleanName = (name || 'Guest').slice(0, 40);
    const wbOnly = !!whiteboardOnly;
    const members = getRoomMembers(roomId);
    const roomIsFresh = members.size === 0 && !roomHosts.has(roomId);

    // First *video* person in an empty room becomes its host and sets the
    // approval policy. A whiteboard-only device (the teacher's tablet) never
    // becomes host — it just joins for drawing.
    if (roomIsFresh && !wbOnly) {
      members.set(socket.id, { name: cleanName });
      rooms.set(roomId, members);
      roomHosts.set(roomId, socket.id);
      roomSettings.set(roomId, { waitingRoomEnabled: !!requireApproval });

      socket.join(roomId);
      joinedRoom = roomId;
      return ack({
        ok: true, peers: [], maxSize: MAX_ROOM_SIZE, isHost: true,
        resumeToken: signResume(roomId),
        whiteboard: whiteboardSnapshot(roomId),
      });
    }

    const settings = roomSettings.get(roomId) || { waitingRoomEnabled: false };
    const hostId = roomHosts.get(roomId);

    // Someone reconnecting after a blip presents a still-valid resume token, so
    // let them straight back in instead of re-queuing them for approval.
    const hasValidResume = !!resumeToken && verifyResume(resumeToken, roomId);

    // Whiteboard-only devices skip the waiting room entirely (it's the
    // teacher's own second device) and join straight away. So do reconnecting
    // members with a valid resume token.
    if (settings.waitingRoomEnabled && hostId && !wbOnly && !hasValidResume) {
      const pending = roomPending.get(roomId) || new Map();
      pending.set(socket.id, { name: cleanName });
      roomPending.set(roomId, pending);
      pendingRoom = roomId;

      ack({ ok: true, waiting: true });
      broadcastPendingList(roomId);
      return;
    }

    if (!wbOnly && videoMemberCount(roomId) >= MAX_ROOM_SIZE) {
      return ack({ ok: false, error: 'room-full', maxSize: MAX_ROOM_SIZE });
    }

    const existingPeers = roomSummary(roomId);
    members.set(socket.id, { name: cleanName, whiteboardOnly: wbOnly });
    rooms.set(roomId, members);

    socket.join(roomId);
    joinedRoom = roomId;

    // 출석 기록: 화이트보드 전용 기기(선생님 태블릿)는 제외하고 실제 학생 입장만 기록.
    if (!wbOnly) {
      attendance.recordJoin({ name: cleanName, roomId })
        .then((id) => { attendanceId = id; })
        .catch((e) => console.error('[attendance] recordJoin failed:', e.message));
    }

    ack({
      ok: true,
      peers: wbOnly ? [] : existingPeers, // whiteboard device does no WebRTC
      maxSize: MAX_ROOM_SIZE,
      isHost: false,
      whiteboardOnly: wbOnly,
      resumeToken: signResume(roomId),
      sharingPeerId: roomSharingPeer.get(roomId) || null,
      whiteboard: whiteboardSnapshot(roomId),
    });

    // Only announce real video participants to the room (so nobody tries to
    // open a WebRTC connection to the whiteboard-only tablet).
    if (!wbOnly) {
      socket.to(roomId).emit('peer-joined', { id: socket.id, name: cleanName });
    }
  });

  // Host approves or denies someone sitting in the waiting queue.
  socket.on('admission-response', ({ targetId, approve }) => {
    if (!joinedRoom || !targetId) return;
    if (roomHosts.get(joinedRoom) !== socket.id) return; // only the host may decide

    const pending = roomPending.get(joinedRoom);
    if (!pending || !pending.has(targetId)) return;
    const info = pending.get(targetId);
    pending.delete(targetId);
    broadcastPendingList(joinedRoom);

    const targetSocket = io.sockets.sockets.get(targetId);

    if (!approve) {
      io.to(targetId).emit('admission-result', { approved: false });
      return;
    }

    const members = getRoomMembers(joinedRoom);
    if (members.size >= MAX_ROOM_SIZE) {
      io.to(targetId).emit('admission-result', { approved: false, reason: 'room-full' });
      return;
    }

    members.set(targetId, { name: info.name });
    rooms.set(joinedRoom, members);
    if (targetSocket) targetSocket.join(joinedRoom);

    const existingPeers = roomSummary(joinedRoom).filter((p) => p.id !== targetId);
    io.to(targetId).emit('admission-result', {
      approved: true,
      peers: existingPeers,
      maxSize: MAX_ROOM_SIZE,
      resumeToken: signResume(joinedRoom),
      sharingPeerId: roomSharingPeer.get(joinedRoom) || null,
      whiteboard: whiteboardSnapshot(joinedRoom),
    });
    io.to(joinedRoom).except(targetId).emit('peer-joined', { id: targetId, name: info.name });
  });

  // After a client is admitted (either approved by the host, or auto-admitted
  // when the host left), it calls this from its OWN connection so we can set
  // its per-socket room state. This is essential: admission-response runs in
  // the *host's* socket scope and cannot set the newcomer's `joinedRoom`, so
  // without this the newcomer's WebRTC signals would be dropped and they'd
  // never actually connect ("approved but can't enter").
  socket.on('confirm-admission', ({ roomId } = {}, ack) => {
    const members = getRoomMembers(roomId);
    if (roomId && members.has(socket.id)) {
      joinedRoom = roomId;
      pendingRoom = null;
      const info = members.get(socket.id);
      if (info && !info.whiteboardOnly) {
        attendance.recordJoin({ name: info.name, roomId })
          .then((id) => { attendanceId = id; })
          .catch((e) => console.error('[attendance] recordJoin failed:', e.message));
      }
    }
    if (typeof ack === 'function') ack({ ok: !!joinedRoom });
  });

  // Someone waiting gives up before the host responds.
  socket.on('cancel-wait', () => leavePendingQueue());

  // Someone started or stopped screen sharing — let everyone else in the
  // room know so their UI can put that person's tile in the spotlight.
  socket.on('screen-share', ({ sharing }) => {
    if (!joinedRoom) return;
    if (sharing) {
      roomSharingPeer.set(joinedRoom, socket.id);
    } else if (roomSharingPeer.get(joinedRoom) === socket.id) {
      roomSharingPeer.delete(joinedRoom);
    }
    socket.to(joinedRoom).emit('screen-share-status', { id: socket.id, sharing: !!sharing });
  });

  // 마이크 on/off 상태를 방의 다른 사람에게 알려, 참여자 타일에 표시하게 한다.
  socket.on('mic-state', ({ on } = {}) => {
    if (!joinedRoom) return;
    socket.to(joinedRoom).emit('peer-mic-state', { id: socket.id, on: !!on });
  });

  // 공유 읽기(현재 PDF 페이지 영어 낭독)를 방의 다른 사람에게 중계한다.
  socket.on('wb-read', (msg = {}) => {
    if (!joinedRoom) return;
    const action = msg.action === 'play' ? 'play' : 'stop';
    const out = { action };
    if (action === 'play') {
      out.text = String(msg.text || '').slice(0, 8000);
      const rate = Number(msg.rate);
      if (rate >= 0.3 && rate <= 2) out.rate = rate;
    }
    socket.to(joinedRoom).emit('wb-read', out);
  });

  // 듣기시험 모드 on/off를 방의 다른 사람(학생)에게 중계한다.
  socket.on('listening-mode', ({ on } = {}) => {
    if (!joinedRoom) return;
    socket.to(joinedRoom).emit('listening-mode', { on: !!on });
  });

  // --- Shared multi-page whiteboard ---
  // Opening/closing the board (shows it big for everyone, like a spotlight).
  socket.on('wb-open', () => {
    if (!joinedRoom) return;
    getWhiteboard(joinedRoom).active = true;
    io.to(joinedRoom).emit('wb-active', { active: true });
  });
  socket.on('wb-close', () => {
    if (!joinedRoom) return;
    getWhiteboard(joinedRoom).active = false;
    io.to(joinedRoom).emit('wb-active', { active: false });
  });

  function findPage(wb, pageId) {
    return wb.pages.find((p) => p.id === pageId);
  }

  // Append the pages of an uploaded PDF as new boards. The client uploaded the
  // bytes over HTTP first, learned the page count + aspect ratios with PDF.js,
  // and now tells us how to lay them out.
  socket.on('wb-add-pdf', ({ pdfId, pages } = {}) => {
    if (!joinedRoom || !pdfId || !Array.isArray(pages)) return;
    const wb = getWhiteboard(joinedRoom);
    // Drop the initial empty blank page if it's the only thing there and unused.
    if (wb.pages.length === 1 && wb.pages[0].type === 'blank' && wb.pages[0].strokes.size === 0) {
      wb.pages = [];
    }
    const room = MAX_PAGES_PER_ROOM - wb.pages.length; // remaining page slots
    pages.slice(0, Math.max(0, Math.min(100, room))).forEach((p, i) => {
      wb.pages.push({
        id: 'pg-' + Math.random().toString(36).slice(2, 9),
        type: 'pdf',
        pdfId: String(pdfId).slice(0, 40),
        pageIndex: Number(p.pageIndex) || (i + 1),
        aspect: Number(p.aspect) || (3 / 4),
        strokes: new Map(),
      });
    });
    io.to(joinedRoom).emit('wb-pages', pageListPayload(joinedRoom));
  });

  socket.on('wb-add-blank', () => {
    if (!joinedRoom) return;
    const wb = getWhiteboard(joinedRoom);
    if (wb.pages.length >= MAX_PAGES_PER_ROOM) return;
    wb.pages.push(newBlankPage());
    io.to(joinedRoom).emit('wb-pages', pageListPayload(joinedRoom));
  });

  // 현재 페이지 한 장 삭제(최소 1장은 유지).
  socket.on('wb-del-page', ({ index } = {}) => {
    if (!joinedRoom) return;
    const wb = getWhiteboard(joinedRoom);
    const n = Number(index);
    if (!Number.isInteger(n) || n < 0 || n >= wb.pages.length) return;
    if (wb.pages.length <= 1) return;
    wb.pages.splice(n, 1);
    if (wb.currentPage >= wb.pages.length) wb.currentPage = wb.pages.length - 1;
    io.to(joinedRoom).emit('wb-pages', pageListPayload(joinedRoom));
    io.to(joinedRoom).emit('wb-page', { index: wb.currentPage });
  });

  // 특정 PDF의 모든 페이지 삭제(+ 서버 보관 중인 PDF 버퍼 해제).
  socket.on('wb-del-pdf', ({ pdfId } = {}) => {
    if (!joinedRoom || !pdfId) return;
    const wb = getWhiteboard(joinedRoom);
    wb.pages = wb.pages.filter((p) => p.pdfId !== pdfId);
    if (!wb.pages.length) wb.pages = [newBlankPage()];
    if (wb.currentPage >= wb.pages.length) wb.currentPage = wb.pages.length - 1;
    const pdfs = roomPdfs.get(joinedRoom);
    if (pdfs) pdfs.delete(pdfId);
    io.to(joinedRoom).emit('wb-pages', pageListPayload(joinedRoom));
    io.to(joinedRoom).emit('wb-page', { index: wb.currentPage });
  });

  // Navigate to a page (broadcast so everyone follows the teacher).
  socket.on('wb-page', ({ index } = {}) => {
    if (!joinedRoom) return;
    const wb = getWhiteboard(joinedRoom);
    const n = Number(index);
    if (!Number.isInteger(n) || n < 0 || n >= wb.pages.length) return;
    wb.currentPage = n;
    io.to(joinedRoom).emit('wb-page', { index: n });
  });

  // Zoom / pan the board (broadcast so everyone sees the same region). Stored so
  // late joiners land on the current view too.
  socket.on('wb-view', ({ zoom, panX, panY } = {}) => {
    if (!joinedRoom) return;
    const wb = getWhiteboard(joinedRoom);
    const z = Math.min(4, Math.max(1, Number(zoom) || 1));
    const px = Math.min(1, Math.max(-1, Number(panX) || 0));
    const py = Math.min(1, Math.max(-1, Number(panY) || 0));
    wb.view = { zoom: z, panX: px, panY: py };
    socket.to(joinedRoom).emit('wb-view', wb.view);
  });

  // A drawing update on a specific page. `points` are normalized 0..1 within
  // that page's rectangle so handwriting lands in the same spot everywhere.
  socket.on('wb-stroke', (msg) => {
    if (!joinedRoom || !msg || !msg.id || !msg.pageId || !Array.isArray(msg.points)) return;
    const wb = getWhiteboard(joinedRoom);
    const page = findPage(wb, msg.pageId);
    if (!page) return;
    let stroke = page.strokes.get(msg.id);
    if (!stroke) {
      stroke = {
        id: String(msg.id).slice(0, 60),
        color: typeof msg.color === 'string' ? msg.color.slice(0, 24) : '#111111',
        width: Number(msg.width) || 3,
        erase: !!msg.erase,
        highlight: !!msg.highlight,
        points: [],
      };
      page.strokes.set(stroke.id, stroke);
      if (page.strokes.size > 5000) {
        const oldestKey = page.strokes.keys().next().value;
        page.strokes.delete(oldestKey);
      }
    }
    const incoming = msg.points.slice(0, 500);
    let added = 0;
    for (const p of incoming) {
      if (p && typeof p.x === 'number' && typeof p.y === 'number') {
        // Stop storing once a single stroke gets pathologically long. The point
        // is still relayed to peers below; only the retained copy is capped.
        if (stroke.points.length >= MAX_POINTS_PER_STROKE) break;
        stroke.points.push({ x: p.x, y: p.y });
        added += 1;
      }
    }
    // Amortized budget check: recount + evict oldest strokes every ~2000 points
    // added, so the board can never grow past MAX_POINTS_PER_ROOM.
    wb._sinceTrim = (wb._sinceTrim || 0) + added;
    if (wb._sinceTrim >= 2000) { wb._sinceTrim = 0; trimWhiteboard(wb); }
    socket.to(joinedRoom).emit('wb-stroke', {
      pageId: msg.pageId, id: stroke.id, color: stroke.color, width: stroke.width,
      erase: stroke.erase, highlight: stroke.highlight, points: incoming, done: !!msg.done,
    });
  });

  // Clear one page (or all pages if pageId omitted).
  socket.on('wb-clear', ({ pageId } = {}) => {
    if (!joinedRoom) return;
    const wb = getWhiteboard(joinedRoom);
    if (pageId) {
      const page = findPage(wb, pageId);
      if (page) page.strokes.clear();
    } else {
      wb.pages.forEach((p) => p.strokes.clear());
    }
    io.to(joinedRoom).emit('wb-clear', { pageId: pageId || null });
  });

  // --- WebRTC signaling relay (server just forwards these blind) ---
  socket.on('signal', ({ to, data }) => {
    if (!to || !joinedRoom) return;
    io.to(to).emit('signal', { from: socket.id, data });
  });

  // --- Simple room chat ---
  socket.on('chat-message', ({ text }) => {
    if (!joinedRoom || !text) return;
    const members = getRoomMembers(joinedRoom);
    const name = members.get(socket.id)?.name || 'Guest';
    const trimmed = String(text).slice(0, 1000);
    io.to(joinedRoom).emit('chat-message', {
      from: socket.id,
      name,
      text: trimmed,
      at: Date.now(),
    });
  });

  socket.on('leave-room', () => leaveCurrentRoom());
  socket.on('disconnect', () => {
    leaveCurrentRoom();
    leavePendingQueue();
  });

  function leavePendingQueue() {
    if (!pendingRoom) return;
    const pending = roomPending.get(pendingRoom);
    if (pending) {
      pending.delete(socket.id);
      broadcastPendingList(pendingRoom);
    }
    pendingRoom = null;
  }

  function leaveCurrentRoom() {
    if (!joinedRoom) return;
    const roomId = joinedRoom;
    const members = getRoomMembers(roomId);
    members.delete(socket.id);

    if (attendanceId) {
      attendance.recordLeave(attendanceId).catch((e) => console.error('[attendance] recordLeave failed:', e.message));
      attendanceId = null;
    }

    // If the person leaving was the one screen-sharing, clear that state so
    // remaining participants' UI drops out of spotlight mode.
    if (roomSharingPeer.get(roomId) === socket.id) {
      roomSharingPeer.delete(roomId);
      socket.to(roomId).emit('screen-share-status', { id: socket.id, sharing: false });
    }

    const wasHost = roomHosts.get(roomId) === socket.id;

    if (members.size === 0) {
      // Room is fully empty — reset it so the next joiner starts fresh
      // (and can become host again if they choose to).
      rooms.delete(roomId);
      roomHosts.delete(roomId);
      roomSettings.delete(roomId);
      roomPending.delete(roomId);
      roomSharingPeer.delete(roomId);
      roomWhiteboard.delete(roomId);
      roomPdfs.delete(roomId);
    } else {
      rooms.set(roomId, members);
      if (wasHost) {
        // The host left but others are still on the call. Nobody's left
        // to approve anyone, so open the room up rather than leaving
        // future joiners stuck forever — and let anyone already waiting
        // in immediately.
        roomHosts.delete(roomId);
        roomSettings.set(roomId, { waitingRoomEnabled: false });
        const pending = roomPending.get(roomId);
        if (pending && pending.size > 0) {
          pending.forEach((info, pendingId) => {
            const pendingSocket = io.sockets.sockets.get(pendingId);
            if (!pendingSocket) return;
            members.set(pendingId, { name: info.name });
            pendingSocket.join(roomId);
            io.to(pendingId).emit('admission-result', {
              approved: true,
              peers: roomSummary(roomId).filter((p) => p.id !== pendingId),
              maxSize: MAX_ROOM_SIZE,
              resumeToken: signResume(roomId),
              sharingPeerId: roomSharingPeer.get(roomId) || null,
              whiteboard: whiteboardSnapshot(roomId),
            });
          });
          rooms.set(roomId, members);
          roomPending.delete(roomId);
        }
      }
    }

    socket.to(roomId).emit('peer-left', { id: socket.id });
    socket.leave(roomId);
    joinedRoom = null;
  }
});

server.listen(PORT, () => {
  console.log(`Video classroom signaling server listening on http://localhost:${PORT}`);
});

db.initSchema()
  .then((ready) => {
    if (!ready) return;
    attendance.startScheduler();
    return analytics.initSchema();
  })
  .catch((e) => console.error('[db] schema init failed:', e.message));
