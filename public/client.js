...
const wbTouchPts = new Map(); // 활성 터치 포인터: id -> {x, y} (client CSS px)
...
function wbPointerDown(e) {
  ...
  if (e.pointerType === 'touch' && wbPenDownId === null) {
    wbTouchPts.set(e.pointerId, { x: e.clientX, y: e.clientY });
...
document.querySelectorAll('.wb-dropdown').forEach((dd) => {
  ...
  // 메뉴 안의 항목을 고르면 닫는다. (단, '더보기'는 굵기 슬라이더·줌 반복 조작을
  //  위해 안쪽 클릭으로 닫지 않고, 바깥 클릭이나 토글로만 닫는다.)
  if (menu && dd.id !== 'wb-more-dd') menu.addEventListener('click', () => wbCloseAllDropdowns());
});
// 바깥을 클릭하면 닫는다.
document.addEventListener('click', wbCloseAllDropdowns);

// ---- 반응형 툴바: 한 줄을 유지하고, 넘치면 덜 쓰는 항목만 '더보기'로 접는다 ----
// 접는 대상은 모두 '평범한 요소'(버튼/라벨/바)라 중첩 드롭다운 없이 이동한다.
// 원래 자리에 주석 노드를 남겨 두어, 공간이 생기면 정확히 되돌린다.
...
function wbRegisterCollapsible(el) {
  if (!el || !el.parentNode) return;
  const slot = document.createComment('wb-slot');
  el.parentNode.insertBefore(slot, el);
  wbCollapsibles.push({ el, slot });
}
// 접기 우선순위(먼저 접히는 순서): 페이지·줌 바 → 굵기 → 이동 → 지우개 → 형광펜 → 연필.
// 색·펜은 항상 남긴다.
[
  document.querySelector('.wb-pagebar'),
  document.querySelector('.wb-size-label'),
  document.getElementById('wb-pan-btn'),
  document.getElementById('wb-eraser-btn'),
  document.getElementById('wb-hi-btn'),
  document.getElementById('wb-pencil-btn'),
].forEach(wbRegisterCollapsible);
...
function wbToolbarWraps() {
  ...
  return kids.some((c) => c.offsetTop - top0 > 20); // 같은 줄 정렬 오차(≤~6px)와 구분
}
...
function reflowToolbar(force) {
  ...
  if (!force && w === wbLastReflowW) return; // 폭이 그대로면 재계산 안 함(깜빡임·루프 방지)
  wbLastReflowW = w;
  // 1) 일단 전부 펼친다.
  wbCollapsibles.forEach(({ el, slot }) => {
    if (el && slot && el.parentNode === wbMoreMenu) slot.after(el);
  });
  wbMoreDd.classList.add('hidden');
  // 2) 한 줄에 안 들어가면 우선순위 낮은 것부터 '더보기'로 접는다.
  for (const { el } of wbCollapsibles) {
    if (!wbToolbarWraps()) break;
    if (el) { wbMoreMenu.appendChild(el); wbMoreDd.classList.remove('hidden'); }
  }
...
async function wbReadSelection(rect) {
  ...
  wbReadOwner = true;
  wbSpeakText(text);
  socket.emit('wb-read', { action: 'play', text: text.slice(0, 8000) });
}
...
async function wbReadWordAt(x, y) {
  ...
  wbReadOwner = true;
  wbSpeakText(word);
  socket.emit('wb-read', { action: 'play', text: word.slice(0, 200) });
}
...
async function wbToggleReading() {
  ...
  if (wbPageTextCache.has(pg.id)) { // 캐시 있으면 제스처 안에서 즉시 재생
    ...
    wbReadOwner = true;
    wbSpeakText(t);
    socket.emit('wb-read', { action: 'play', text: t.slice(0, 8000) }); // 모두에게 동시 재생
    return;
  }
  ...
  wbReadOwner = true;
  wbSpeakText(t);
  socket.emit('wb-read', { action: 'play', text: t.slice(0, 8000) });
}
