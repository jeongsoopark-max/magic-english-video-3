...
if (soundToggleBtn) {
  soundToggleBtn.addEventListener('click', () => {
    approvalSoundOn = !approvalSoundOn;
    soundToggleBtn.textContent = approvalSoundOn ? '🔔 알림음' : '🔕 알림음 꺼짐';
    if (approvalSoundOn) playApprovalChime();   // preview + unlocks audio
  });
}
...
function enterCallScreen(peers, sharingPeerId, whiteboard) {
  ...
  if (soundToggleBtn) soundToggleBtn.classList.toggle('hidden', !isHost);
  if (!isHost) {
...
// --- Approval chime -------------------------------------------------------
// A short two-tone chime plays when someone new asks to be let in, so the
// teacher notices even while writing on the board or looking elsewhere.
// Generated with WebAudio so no sound file is needed.
let wbAudioCtx = null;
let lastPendingCount = 0;
let approvalSoundOn = true;
function playApprovalChime() {
  if (!approvalSoundOn) return;
  try {
    const AC = window.AudioContext || window.webkitAudioContext;
    if (!AC) return;
    if (!wbAudioCtx) wbAudioCtx = new AC();
    if (wbAudioCtx.state === 'suspended') wbAudioCtx.resume();
    const now = wbAudioCtx.currentTime;
    // Two rising notes (E6 → A6), soft bell-like envelope.
    [[1318.5, 0], [1760, 0.15]].forEach(([freq, offset]) => {
      const osc = wbAudioCtx.createOscillator();
      const gain = wbAudioCtx.createGain();
      osc.type = 'sine';
      ...
      osc.connect(gain).connect(wbAudioCtx.destination);
      osc.start(t);
      osc.stop(t + 0.45);
    });
...
socket.on('pending-list', (list) => {
  pendingList = list || [];
  // Ring only when the queue grows (a genuinely new request), not on approvals.
  if (pendingList.length > lastPendingCount) {
    playApprovalChime();
    const who = pendingList[pendingList.length - 1];
    triggerToastIfAvailable(`${(who && who.name) || '학생'}님이 입장을 요청했습니다.`);
  }
...
// ---- Whiteboard participant-strip layout (bottom/right + resize) ----------
// Same controls as the screen-share spotlight, but here the "featured" element
// is the whiteboard panel (a sibling of the video grid inside call-main), so
// we resize call-main's split instead of a tile inside the grid.
let wbThumbPosition = 'bottom'; // 'bottom' | 'right'
let wbThumbBottomSize = 130;    // px height when the strip is at the bottom
let wbThumbRightSize = 240;     // px width when the strip is on the right

(function loadWbLayoutPrefs() {
  try {
    const s = JSON.parse(localStorage.getItem('vc_wb_layout_prefs') || 'null');
    if (!s) return;
    if (s.position === 'bottom' || s.position === 'right') wbThumbPosition = s.position;
    if (typeof s.bottomSize === 'number') wbThumbBottomSize = clamp(s.bottomSize, 90, 360);
    if (typeof s.rightSize === 'number') wbThumbRightSize = clamp(s.rightSize, 160, 520);
  } catch (e) { /* defaults */ }
})();
function saveWbLayoutPrefs() {
  try {
    localStorage.setItem('vc_wb_layout_prefs', JSON.stringify({
      position: wbThumbPosition, bottomSize: wbThumbBottomSize, rightSize: wbThumbRightSize
    }));
  } catch (e) { /* ignore */ }
}
function applyWbThumbSizeVar() {
  if (!callMain) return;
  const size = wbThumbPosition === 'right' ? wbThumbRightSize : wbThumbBottomSize;
  callMain.style.setProperty('--wb-thumb-size', `${size}px`);
}
function ensureWbResizeHandle() {
  if (!callMain || !videoGrid) return;
  ...
  callMain.insertBefore(handle, videoGrid); // sits between board and strip
}
function removeWbResizeHandle() {
  const handle = document.getElementById('wb-resize-handle');
  if (handle) handle.remove();
  if (callMain) callMain.style.removeProperty('--wb-thumb-size');
}
function updateWbLayout() {
  if (!callMain) return;
  ...
  applyWbThumbSizeVar();
  ensureWbResizeHandle();
  if (wbActive) requestAnimationFrame(() => { sizeWhiteboardCanvas(); renderCurrentPage(); });
}
function setWbThumbPosition(pos) {
  if (pos !== 'bottom' && pos !== 'right') return;
...
function attachWbResizeHandlers(handle) {
  ...
  function down(e) {
    ...
    startSize = wbThumbPosition === 'right' ? wbThumbRightSize : wbThumbBottomSize;
  ...
  function move(e) {
    if (!dragging) return;
    const p = pt(e);
    const current = wbThumbPosition === 'right' ? p.clientX : p.clientY;
    const delta = startPos - current; // drag toward the board = grow strip
    if (wbThumbPosition === 'right') wbThumbRightSize = clamp(startSize + delta, 160, 520);
    else wbThumbBottomSize = clamp(startSize + delta, 90, 360);
    applyWbThumbSizeVar();   // cheap during drag; canvas re-render happens on release
    e.preventDefault();
  }
  function up() {
    if (!dragging) return;
...
if (wbLayoutBottomBtn) wbLayoutBottomBtn.addEventListener('click', () => setWbThumbPosition('bottom'));
if (wbLayoutRightBtn) wbLayoutRightBtn.addEventListener('click', () => setWbThumbPosition('right'));
...
function drawStrokeSegment(stroke, fromIdx) {
  ...
  } else if (stroke.pencil) {
    // Pencil: thin, slightly translucent graphite look.
    wbCtx.globalCompositeOperation = 'source-over';
    wbCtx.globalAlpha = 0.75;
    wbCtx.strokeStyle = stroke.color;
  } else {
    wbCtx.globalCompositeOperation = 'source-over';
    wbCtx.strokeStyle = stroke.color;
  ...
  wbCtx.lineWidth = stroke.width * (r.h / 800) * (window.devicePixelRatio || 1) + 0.2;
...
function wbPointerDown(e) {
  ...
  // Divide by the current zoom so the on-screen thickness always matches the
  // slider: writing fine details on a zoomed-in PDF no longer comes out huge.
  const z = wbZoom || 1;
  const baseWidth = isEraser ? Math.max(wbSize * 2.5, 8)
    : isHi ? Math.max(wbSize * 2.2, 5)
    : isPencil ? Math.max(wbSize * 0.6, 0.3)
    : wbSize;
  const stroke = {
    color: wbColor,
    width: baseWidth / z,
...
function wbFlushSend(done) {
  ...
  socket.emit('wb-stroke', {
    ...
    erase: stroke.erase, highlight: stroke.highlight, pencil: stroke.pencil,
    points: wbSendBuffer, done: !!done,
  });
  wbSendBuffer = [];
}

// ---- Receiving strokes from others ----
socket.on('wb-stroke', ({ pageId, id, color, width, erase, highlight, pencil, points }) => {
  if (!pageId || !id || !Array.isArray(points)) return;
  const pg = wbPages.find((p) => p.id === pageId);
  if (!pg) return;
  let stroke = pg.strokes.get(id);
  if (!stroke) {
    stroke = { color: color || '#111', width: width || 3, erase: !!erase, highlight: !!highlight, pencil: !!pencil, points: [] };
...
function applyWhiteboardSnapshot(snapshot) {
  ...
  wbPages = (snapshot.pages || []).map((pg) => {
    ...
    (pg.strokes || []).forEach((s) => {
      strokes.set(s.id, { color: s.color, width: s.width, erase: !!s.erase, highlight: !!s.highlight, pencil: !!s.pencil, points: s.points || [] });
    });
...
function showWhiteboard() {
  ...
  updateWbLayout();   // apply bottom/right strip + resize handle
  requestAnimationFrame(() => { sizeWhiteboardCanvas(); renderCurrentPage(); });
}
function hideWhiteboard() {
  wbActive = false;
  whiteboardPanel.classList.add('hidden');
  if (callMain) callMain.classList.remove('wb-active', 'wb-thumb-right');
  removeWbResizeHandle();
...
async function renderPageToCanvas(pg, maxW) {
  ...
  pg.strokes.forEach((stroke) => {
    ...
    ctx.globalAlpha = stroke.highlight ? 0.4 : (stroke.pencil ? 0.75 : 1);
    ctx.strokeStyle = stroke.erase ? '#ffffff' : stroke.color;
    ctx.fillStyle = ctx.strokeStyle;
    ctx.lineWidth = stroke.width * (out.height / 800) + 0.25;
    if (pts.length === 1) {
      ctx.beginPath();
...
// Save a blob straight to the device (no share sheet). Used by "전체 PDF 저장".
...
// Build a single PDF (all whiteboard pages, background + strokes) and return it
// as a Blob. Shared by both the "저장"(download) and "공유"(share sheet) buttons.
async function buildWholePdfBlob() {
...
// deliver: 'download' saves the file to the device; 'share' opens the native
// share sheet (KakaoTalk etc.) and falls back to download where unsupported.
async function exportWholePdf(deliver) {
  ...
  try {
    ...
    const blob = await buildWholePdfBlob();
    const stamp = new Date().toISOString().slice(0, 10);
    const filename = `수업필기_${stamp}.pdf`;
    if (deliver === 'download') {
      downloadBlob(blob, filename);
      triggerToastIfAvailable('전체 PDF를 저장했어요.');
    } else {
      await shareOrDownload(blob, filename, '수업 필기');
    }
  } catch (e) {
...
if (wbSharePdfBtn) {
  wbSharePdfBtn.addEventListener('click', () => exportWholePdf('download'));
}
if (wbSharePdfFileBtn) {
  wbSharePdfFileBtn.addEventListener('click', () => exportWholePdf('share'));
}
...
if (wbSizeInput) wbSizeInput.addEventListener('input', () => {
  const v = parseFloat(wbSizeInput.value);
  wbSize = (isFinite(v) && v > 0) ? v : 3;
});
