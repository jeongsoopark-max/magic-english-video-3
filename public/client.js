...
// 페이지·줌 컨트롤(이전/다음/−/%/+/맞춤)을 '페이지' 드롭다운 안으로 이동.
(function () {
  const pageMenu = document.querySelector('#wb-page-dd .wb-dd-menu');
  const pagebar = document.querySelector('.wb-pagebar');
  if (pageMenu && pagebar) {
    const sep = document.createElement('div');
    sep.className = 'wb-dd-sep';
    pageMenu.appendChild(sep);
    while (pagebar.firstChild) pageMenu.appendChild(pagebar.firstChild);
    pagebar.remove(); // 빈 껍데기 제거
  }
})();

// 접을 대상 없음(그리기 도구는 항상 상단, 페이지/줌은 '페이지' 안, 듣기는 '더보기' 안).
