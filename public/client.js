...
// ---- 수업 녹화 (화면 + 소리) ----------------------------------------------
// 🔴 버튼: PC(크롬·엣지)에서 화면/탭/창을 골라 녹화하고, 교사 마이크 소리를 함께
// 섞어 하나의 .webm 파일로 저장한다. 공유 대화상자에서 '이 탭'을 소리 포함으로
// 고르면, 탭에서 재생되는 학생들 목소리(WebRTC 오디오)까지 함께 녹음된다.
// iPad/iPhone Safari는 getDisplayMedia 미지원 → 안내 후 iOS 화면 기록 사용 권장.
let wbRec = null; // { recorder, chunks, streams, audioCtx }
function wbIsRecording() {
  return !!(wbRec && wbRec.recorder && wbRec.recorder.state === 'recording');
}
function wbPickRecMime() {
...
function wbSetRecordUI(recording) {
  const btn = document.getElementById('wb-record-btn');
  if (!btn) return;
  if (recording) {
    btn.textContent = '⏹';
    btn.title = '녹화 중지 (클릭하면 파일로 저장)';
    btn.setAttribute('aria-label', '녹화 중지');
    btn.style.background = '#e5484d';
    btn.style.color = '#fff';
    btn.style.borderColor = '#e5484d';
  } else {
    btn.textContent = '🔴';
    btn.title = '수업 녹화 (화면 + 소리를 녹화해 파일로 저장)';
    btn.setAttribute('aria-label', '수업 녹화');
...
async function wbStartRecording() {
  if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia || !window.MediaRecorder) {
    alert('이 기기에서는 브라우저 화면 녹화를 지원하지 않아요.\n\n· 아이패드/아이폰: 제어센터의 "화면 기록"으로 녹화하세요.\n· PC(크롬·엣지): 이 버튼으로 바로 녹화됩니다.');
    return;
  }
  let display;
  let mic = null;
  ...
  } catch (_) { return; } // 사용자가 화면 선택을 취소함
  try {
    mic = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
  } catch (_) { mic = null; } // 마이크가 없거나 거부됨 → 화면 소리만

  // 오디오 믹싱: 화면(탭/시스템) 소리 + 마이크 → 하나의 트랙으로 합침
  let audioTracks = [];
  let audioCtx = null;
  const AudioCtx = window.AudioContext || window.webkitAudioContext;
  const hasDispAudio = display.getAudioTracks().length > 0;
  if (AudioCtx && (hasDispAudio || mic)) {
    audioCtx = new AudioCtx();
    try { await audioCtx.resume(); } catch (_) {}
    const dest = audioCtx.createMediaStreamDestination();
    if (hasDispAudio) audioCtx.createMediaStreamSource(new MediaStream([display.getAudioTracks()[0]])).connect(dest);
    if (mic) audioCtx.createMediaStreamSource(mic).connect(dest);
    audioTracks = dest.stream.getAudioTracks();
  } else if (hasDispAudio) {
    audioTracks = [display.getAudioTracks()[0]];
  }

  const videoTrack = display.getVideoTracks()[0];
  const mixed = new MediaStream([videoTrack].concat(audioTracks));
  const mime = wbPickRecMime();
  let recorder;
  try {
    recorder = new MediaRecorder(mixed, mime ? { mimeType: mime } : undefined);
  } catch (e) {
    alert('이 브라우저에서 녹화를 시작할 수 없어요. 크롬·엣지 최신 버전을 권장합니다.');
    [display, mic].forEach((s) => s && s.getTracks().forEach((t) => t.stop()));
    return;
  }
  wbRec = { recorder, chunks: [], streams: [display, mic].filter(Boolean), audioCtx };
  recorder.ondataavailable = (e) => { if (e.data && e.data.size) wbRec.chunks.push(e.data); };
  recorder.onstop = wbFinalizeRecording;
  recorder.start(1000); // 1초 단위로 청크를 모아 긴 녹화에도 안전
  // 브라우저 자체 '공유 중지' 버튼으로 화면 공유를 끊으면 녹화도 자동 종료.
  videoTrack.addEventListener('ended', () => { if (wbIsRecording()) wbStopRecording(); });
  wbSetRecordUI(true);
}
function wbStopRecording() {
  try {
    if (wbRec && wbRec.recorder && wbRec.recorder.state !== 'inactive') wbRec.recorder.stop();
  } catch (_) {}
}
function wbFinalizeRecording() {
  const rec = wbRec;
  wbRec = null;
  wbSetRecordUI(false);
  if (!rec) return;
  rec.streams.forEach((s) => { try { s.getTracks().forEach((t) => t.stop()); } catch (_) {} });
  if (rec.audioCtx) { try { rec.audioCtx.close(); } catch (_) {} }
  const type = (rec.recorder && rec.recorder.mimeType) || 'video/webm';
  const blob = new Blob(rec.chunks, { type });
  if (!blob.size) return;
  const ext = type.indexOf('mp4') >= 0 ? 'mp4' : 'webm';
  const t = new Date();
  const p = (n) => String(n).padStart(2, '0');
  const name = `수업녹화_${t.getFullYear()}${p(t.getMonth() + 1)}${p(t.getDate())}_${p(t.getHours())}${p(t.getMinutes())}.${ext}`;
...
(function wireRecordButton() {
  const btn = document.getElementById('wb-record-btn');
  if (!btn) return;
  btn.addEventListener('click', () => { if (wbIsRecording()) wbStopRecording(); else wbStartRecording(); });
})();
